...
Neste post apresento uma introdução sobre bibliote criada por [Seth Juarez][1] com o objetivo de auxiliar o use de algoritmos comuns de machine learning com a plataforma .NET. Esta biblioteca chama-se [numl][3]. Pela simplicidade e alto nível de abstração dessa bibliote, fica claro que o objetivo de Seth é quebrar a barreira da iniciação por iniciante. Mesmo que você não entenda os principais algoritmos utilizado em machine learning (Similaridade de Cosseno, Distância Hamming, entre outros), é possível, *com poucas linha* criar um programa para prever 
